...
class TTSProcessor:
    ...
    def __init__(self):
        ...
        self.speaker = self.config.tts_speaker
        self._load_model()

    def _load_model(self):
        """加载TTS模型"""
        try:
            tts_logger.info(f"正在加载TTS模型: {self.model_name}")
            ...
            self.model = TTS(
                ...
                progress_bar=False,
                gpu=self.config.use_gpu and torch.cuda.is_available(),
            )

            tts_logger.info("TTS模型加载成功")

        except Exception as e:
            tts_logger.error(f"TTS模型加载失败: {e}")
            raise
    ...
    def synthesize_speech(self, text: str, output_path: Optional[str] = None) -> str:
        ...
        try:
            if not text.strip():
                tts_logger.warning("输入文本为空，跳过语音合成")
                return ""

            tts_logger.info(f"开始合成语音，文本: {text}")

            # 如果没有指定输出路径，生成临时文件路径
            if output_path is None:
                output_path = os.path.join(self.config.temp_dir, "tts_output.wav")
            ...
            self.model.tts_to_file(
                ...
                speaker_wav=self.config.reference_audio,
                language=self.language,
                split_sentences=True,
            )

            tts_logger.info(f"语音合成完成，输出文件: {output_path}")
            return output_path
    ...
    def synthesize_speech_with_reference(
        ...
        try:
            ...
            if not os.path.exists(reference_audio):
                tts_logger.error(f"参考音频文件不存在: {reference_audio}")
                return ""

            tts_logger.info(f"开始使用参考音频合成语音，文本: {text}")

            # 如果没有指定输出路径，生成临时文件路径
            if output_path is None:
                output_path = os.path.join(
                    self.config.temp_dir, "tts_cloned_output.wav"
                )
            ...
            tts_logger.info(f"语音克隆合成完成，输出文件: {output_path}")
            return output_path

        except Exception as e:
            tts_logger.error(f"语音克隆合成失败: {e}")
            return ""
    ...
    def synthesize_to_array(self, text: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            tts_logger.info(f"合成语音到数组，文本: {text}")

            # 生成临时文件
            temp_file = os.path.join(self.config.temp_dir, "temp_tts.wav")
            ...
            tts_logger.error(f"合成音频到数组失败: {e}")
            return None
    ...
    def get_available_speakers(self) -> list:
        ...
        try:
            if hasattr(self.model, "speakers") and self.model.speakers:
                ...
                tts_logger.warning("无法获取说话人列表")
                return []
        except Exception as e:
            tts_logger.error(f"获取说话人列表失败: {e}")
            return []
    ...
    def get_available_languages(self) -> list:
        ...
        try:
            if hasattr(self.model, "languages") and self.model.languages:
                ...
                tts_logger.warning("无法获取语言列表")
                return ["zh", "en"]  # 默认支持中英文
        except Exception as e:
            tts_logger.error(f"获取语言列表失败: {e}")
            return ["zh", "en"]

    def set_speaker(self, speaker: str):
        """设置说话人"""
        self.speaker = speaker
        tts_logger.info(f"TTS说话人设置为: {speaker}")

    def set_language(self, language: str):
        """设置语言"""
        self.language = language
        tts_logger.info(f"TTS语言设置为: {language}")
